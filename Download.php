...
//            copy('phar://mda.phar/config.ini', $this->localConfigFile);
        ...
        foreach (['ID', 'TOKEN'] as $requiredField) {
            if (!isset($this->config[$requiredField])) {
                throw new Exception(sprintf('Magento %s is not set',
                    $requiredField));
            }
        }

        $this->cacheFile = $this->cwd . '/.mda.'
            . sha1(serialize($this->config));

        $url = sprintf($this->downloadUrlFormat, $this->config['ID'],
            $this->config['TOKEN']);
        ...
        if (file_exists($this->cacheFile)
            && (time() - filemtime($this->cacheFile) < 3600)
        ) {
            $downloadsJson = file_get_contents($this->cacheFile);
        } else {
            if ($downloadsJson = file_get_contents($downloadUrl)) {
...
EOF;
    ...
    public function download($filename, $destinationFile = false)
    {
        ...
        if (!$destinationFile) {
            $destinationFile = sprintf('%s/%s/%s', $this->cwd,
                $this->downloadDir, $filename);
        }

        // Try downloading from the Sonassi github repo for better performance
        $githubUrl = sprintf('%s/%s/%s', $this->githubUrlFormat,
            $this->githubDownloadsRepo, $filename);
        ...
        if (file_exists($destinationFile)
            && filesize($destinationFile) == $filesize
        ) {
        ...
        while (!feof($remote)) {
            ...
            echo str_pad(sprintf("%s %s%% of %sMB (%s MB/s)", $msgPrefix,
                    $progress, $filesizeMb, $mbps), 40) . "\r";
        }
        fclose($remote);
        fclose($local);
    ...
    public function downloadFull($magentoVersion = 'ce-full')
    {
        ...
        switch ($downloadRelease) {
            case 'a':
                ...
                $downloadReleases = array_diff_key($downloadMap,
                    array_flip($remove));
                $all = true;
                break;
            case 'q':
        ...
        foreach ($downloadReleases as $downloadRelease) {
            ...
            $downloadMap = [];
            foreach (
                $this->downloadData[$magentoVersion][$downloadRelease] as
                $release
            ) {
            ...
            switch ($downloadFilename) {
                case 'a':
                    $remove = preg_grep('/[^0-9]+/', array_keys($downloadMap));
                    $downloadFilenames = array_diff_key($downloadMap,
                        array_flip($remove));
                    $all = true;
                    break;
                case 'q':
            ...
            foreach ($downloadFilenames as $downloadFilename) {
                printf("\nDownloading %s\n--\n\n", $downloadFilename);
                $downloadFile = sprintf('%s/%s/%s', $this->cwd,
                    $this->downloadDir, $downloadFilename);
                if ($all
                    && preg_match('/(Samples|sample_data)/', $downloadFile)
                ) {
                    $this->log[]
                        = sprintf("Downloads including sample data skipped when downloading all\r\n");
                } else {
                    if ($destinationFile = $this->download($downloadFilename,
                        $downloadFile)
                    ) {
    ...
    public function downloadOther()
    {
        ...
        foreach ($downloadFilenames as $downloadFilename) {
            ...
            $downloadFile = sprintf('%s/%s/%s', $this->cwd, $this->downloadDir,
                $downloadFilename);
            if ($destinationFile = $this->download($downloadFilename,
                $downloadFile)
            ) {
    ...
    public function downloadPatch(
        $downloadRelease = false,
        $magentoVersion = 'ce-patch'
    ) {
        uksort($this->downloadData[$magentoVersion], 'version_compare');
        ...
        $appliedFilename = sprintf("%s/app/etc/applied.patches.list",
            $this->cwd);
        $appliedPatches = [];
        if (file_exists($appliedFilename)) {
            $content = file_get_contents($appliedFilename);
            if (preg_match_all('/^[a-zA-Z0-9:\s\-]+ \| ([^\|]+) \| ([^\|]+) \| ([^\|]+) \|/m',
                $content, $matches)
            ) {
                foreach ($matches[0] as $key => $value) {
                    $patchName = preg_replace([
                        '/^[A-Z_-]+[_-]SUPEE/',
                        '/(SUPEE-[0-9.]+)[_-].+/'
                    ], ['SUPEE', '\\1'], trim($matches[1][$key]));
                    $magentoRelease = trim($matches[2][$key]);
                    $patchVersion = trim($matches[3][$key]);

                    //$appliedPatches[] = $patchName;
                    $appliedPatches[] = sprintf('%s-%s', $patchName,
                        $patchVersion);
        ...
        foreach ($downloadReleases as $downloadRelease) {
            ...
            if (is_array($this->downloadData[$magentoVersion][$downloadRelease])) {
                ...
                foreach (
                    ...
                    $patchVersion = (preg_match('/(v[0-9.]+)/',
                        $release['file_name'], $matches)) ? rtrim($matches[1],
                        '.') : 'v1';
                    $patchName = (preg_match('/(SUPEE-[0-9]+)/',
                        $release['file_name'], $matches))
                        ? $matches[1]
                        : (preg_match('/(SUPEE-[0-9]+)/', $release['name'],
                            $matches))
                            ? $matches[1]
                            : '';
                    $patchCombinedName = sprintf('%s-%s', $patchName,
                        $patchVersion);
                    ...
                    if (in_array($release['file_name'], $appliedPatches)
                        ...
                        $status
                            = $this->colors->getColoredString(str_pad('Installed',
                            12), 'green');
                    } else {
                        $status
                            = $this->colors->getColoredString(str_pad('Missing',
                            12), 'red');
                        $missingPatches[] = $release['file_name'];
                    }

                    $option = sprintf(" [%d]:", $id++);
                    printf("%s %s%s (%s/%s)\n", str_pad($option, 8), $status,
                        $release['name'], $release['file_name'], $shortName);
                }

            }

            foreach ($missingPatches as $downloadFilename) {
                printf("Downloading %s\n--\n\n", $downloadFilename);
                $downloadFile = sprintf('%s/%s/%s', $this->cwd,
                    $this->downloadDir, $downloadFilename);
    ...
    public function interactiveDownload()
    {
        ...
        $downloadMethod = sprintf('download%s',
            ucfirst(str_replace('-', '', $downloadType)));
    ...
    public function getDownloadData($version)
    {
        return $this->downloadData['ce-patch'][$version];
    }
